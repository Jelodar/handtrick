...
Circle is resolved through the path resolver. A four-segment suffix matching `right>down>left>up` from any start emits `circle:cw`; the reverse cycle emits `circle:ccw`. Circle selectors may include count and direction tokens, such as `circle:2x:cw`; finger count belongs in criteria, such as `hand.on('circle:cw', { fingers: 2 }, fn)`. Explicit `1x` canonicalizes away, so `circle:1x:cw` becomes `circle:cw`. The path parser treats circle as an atom, so the same count/direction adjusters work inside longer patterns such as `up>circle:2x:cw` and path criteria. Circle items participate in normal command arbitration for the current semantic batch, while registered longer counts can hold shorter circle commands pending until the path can no longer complete the longer match. `path.maxCircleCount` bounds retained path length for accidental huge count selectors.
